...
- Local app notifications создаются клиентом через Windows App SDK `AppNotificationManager` после local fetch/decrypt/privacy checks.
- Taskbar badge является локальным derived state из unread counters; backend не присылает plaintext для badge.
- Notification activation payload содержит только route-safe identifiers и проходит через local session/lock gate.
- Первый Windows beta может использовать foreground WebSocket + local notifications до появления WNS.
- WNS требует отдельного backend contract и ADR: PFN/channel lifecycle, registration, revocation, delivery semantics и metadata-only payload schema.
